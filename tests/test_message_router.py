...
class MessageRouterTests(unittest.TestCase):
    def test_routes_backend_to_code_reviewer_when_no_frontend(self):
        next_role = route_next_role(Role.BACKEND_DEV, has_frontend=False)
        self.assertEqual(next_role, Role.CODE_REVIEWER)
    ...
    def test_routes_reviewer_to_tester(self):
        next_role = route_next_role(Role.CODE_REVIEWER, has_frontend=True)
        self.assertEqual(next_role, Role.TESTER)

    def test_routes_final_tester_to_none(self):
        next_role = route_next_role(Role.TESTER, has_frontend=True)
